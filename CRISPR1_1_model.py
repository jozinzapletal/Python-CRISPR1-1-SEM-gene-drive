...
def CRISPR_1_1_Progression():
    ...
    seeded_Juveniles = [[0 for x in range(2 * len(cols))] for y in range(span)]
    total_Adults = [[0 for x in range(2 * len(cols))] for y in range(span)]
    total_Juveniles = [[0 for x in range(2 * len(cols))] for y in range(span)]
        ...
        for alpha2 in female_alphas:
            for gamma1 in male_gammas:
                for gamma2 in female_gammas:
                    ...
                    for T in range(span):
                        if T == 0:
                            surviving_Adults[0] = g0
                            proportionPop = convert_to_proportion(g0)
                            seeded_Juveniles[0] = CRISPR1_1_Equations.CRISPR1_1_Rates(proportionPop, sigma, lamda, delta1, delta2, fitCost,
                                                                                      q1, q2, P1, P2, alpha1, alpha2, beta1, beta2, gamma1, gamma2)

                            total_Adults[T] = [i + j for i, j in zip(total_Adults[T], surviving_Adults[T])]
                            total_Juveniles[T] = [i + j for i, j in zip(total_Juveniles[T], seeded_Juveniles[T])]

                        elif 0 < T < developTime:

                            # only the initial adults will be producing offspring at this time.
                            if T < interval2:
                                surviving_Adults[0] = [i * j for i, j in zip(surviving_Adults[0], [(1 - k) for k in mortRateAdult1])]
                            if interval2 <= T <= interval3:
                                surviving_Adults[0] = [i * j for i, j in zip(surviving_Adults[0], [(1 - k) for k in mortRateAdult2])]
                            else:
                                surviving_Adults[0] = [i * j for i, j in zip(surviving_Adults[0], [(1 - k) for k in mortRateAdult3])]

                            proportionPop = convert_to_proportion(surviving_Adults[0])
                            J2 = CRISPR1_1_Equations.CRISPR1_1_Rates(proportionPop, sigma, lamda, delta1, delta2, fitCost, q1, q2,
                                                                     P1, P2, alpha1, alpha2, beta1, beta2, gamma1, gamma2)
                            seeded_Juveniles[T] = [i + j for i, j in zip(seeded_Juveniles[T], J2)]

                            total_Adults[T] = [i + j for i, j in zip(total_Adults[T], surviving_Adults[0])]
                            total_Juveniles[T] = [i + j for i, j in zip(total_Juveniles[T], J2)]

                        else:
                            for T2 in range(max(0, T - previous_days), T):
                                if (T - T2) < interval2:
                                    surviving_Adults[T2] = [i * j for i, j in zip(surviving_Adults[T2], [(1 - k) for k in mortRateAdult1])]
                                if interval2 <= (T - T2) <= interval3:
                                    surviving_Adults[T2] = [i * j for i, j in zip(surviving_Adults[T2], [(1 - k) for k in mortRateAdult2])]
                                else:
                                    surviving_Adults[T2] = [i * j for i, j in zip(surviving_Adults[T2], [(1 - k) for k in mortRateAdult3])]

                                # surviving adults from previous time steps will not mix with newly emerged adults
                                proportionPop = convert_to_proportion(surviving_Adults[T2])
                                J2 = CRISPR1_1_Equations.CRISPR1_1_Rates(proportionPop, sigma, lamda, delta1, delta2, fitCost,
                                                                         q1, q2, P1, P2, alpha1, alpha2, beta1, beta2, gamma1, gamma2)
                                seeded_Juveniles[T] = [i + j for i, j in zip(seeded_Juveniles[T], J2)]

                                total_Adults[T] = [i + j for i, j in zip(total_Adults[T], surviving_Adults[T2])]
                                total_Juveniles[T] = [i + j for i, j in zip(total_Juveniles[T], J2)]

                            # new adults create genotypes of
                            new_adults = [i * j for i, j in zip(seeded_Juveniles[T-developTime], [(1 - k) for k in juvenile_mort_rate])]
                            surviving_Adults[T] = new_adults
                            ...
                            total_Adults[T] = [i + j for i, j in zip(total_Adults[T], new_adults)]
                            total_Juveniles[T] = [i + j for i, j in zip(total_Juveniles[T], J2)]

                        # Store the values of the model into a list
                        total_population_by_genotype[T] = [i + j + k for i, j, k in zip(total_population_by_genotype[T], total_Juveniles[T], total_Adults[T])]
